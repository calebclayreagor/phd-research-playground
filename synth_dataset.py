...
# custom script for adaptive Lasso with initial biasing (ablasso)
from ablasso                               import ablasso, infer_grn, auprc
...
class synthdata:
    ...
    def generate_model(self, n_tfs, n_targets, selfs=0.5, frac=0.5,
                 strengths_min=0.8, strengths_max=1, k_strengths=2,
                        mindist=0.5, load_prev=False, verbose=True):
        """
...
f"""output_settings:
    ...
    output_prefix: '{self.name}' """)
        ...
        if mode=='lasso':
            if load_prev:
                try:
                    self.lasso_grid = pd.read_csv(f'outputs/{self.name}/lasso_grid.csv', index_col=0)
                ...
                if pars=='default':
                    pars = {'alpha' : np.logspace(-10, 0, 11),
                            'beta'  : np.array([1e-1, 0, 2, 4, 8, 10, 12, 14, 16, 18, 20]),
                            'train_split' : 0.7}

                n_train = int(pars['train_split'] * len(self.expression))
                print(f'Performing Lasso with n={n_train} training datasets...')

                # vectorize function for grid search over alpha, beta hyperparameters
                vauprc = np.vectorize(auprc, excluded=['X','weights0','y_true','return_adj'])
                aa, bb = np.meshgrid(pars['alpha'], pars['beta'])
                grid_arr = np.zeros((aa.shape[0], aa.shape[1], n_train))
                for i in tqdm(np.arange(grid_arr.shape[2])):
                    sce = self.expression[i][self.tfbs[i].columns]
                    tfbs_inv = (1/self.tfbs[i]).replace(np.inf, 0)
                    grid_arr[...,i] = vauprc(X=sce, weights0=tfbs_inv, alpha=aa, beta=bb,
                                             y_true=(self.grn_full.values != 0).ravel())

                mean_grid = grid_arr.mean(axis=2).T
                idx = mean_grid.T.ravel().argmax()
                alpha_best = aa.ravel()[idx]
                beta_best = bb.ravel()[idx]

                # grid search results
                self.lasso_grid = pd.DataFrame(mean_grid, index=pars['alpha'], columns=pars['beta'])
                self.lasso_grid.to_csv(f'outputs/{self.name}/lasso_grid.csv')

                # inference results
                self.lasso_auc = pd.DataFrame(columns=[f'{self.name}-{n_cells}-{i+1}-{int(100*drop_cutoff)}-{drop_prob}'
                                                       for i in range(len(self.expression))], index=['ABLASSO', 'split'])
                for i in np.arange(n_train):
                    self.lasso_auc.iloc[:, i] = grid_arr[...,i].ravel()[idx], 'train'

                for i in np.arange(n_train, len(self.expression)):
                    sce = self.expression[i][self.tfbs[i].columns]
                    tfbs_inv = (1/self.tfbs[i]).replace(np.inf, 0)
                    self.lasso_auc.iloc[:, i] = auprc(X=sce, weights0=tfbs_inv, alpha=alpha_best, beta=beta_best,
                                                                      y_true=(self.grn_full.values != 0).ravel()), 'dev'
