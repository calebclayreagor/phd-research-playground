...
# from bblocks.py
from scripts.bblocks         import bayesian_blocks
...
#        self.umap_embedding       = None
    ...
    def preprocess_raw_counts(self, library_size_cutoff=0):
        ...
        * dataset.normalized: pd.DataFrame, filtered, normalized and scaled
        ...
        self.normalized = filter_library_size(self.raw_counts,
                          cutoff=library_size_cutoff)
        self.normalized = filter_rare_genes(self.normalized)
    ...
    def find_gene_similarities(self, n_runs=5, seed=0):
        """
    ...
    def cluster_genes(
        self, min_clusters=2, max_clusters=20, n_components=10, plot_silhouette=True, seed=0):
        """
    ...
    def embed_pca(self, data='raw_counts', genes=[], n_components=20, seed=0):
        """
    ...
    def embed_tsne(self, data='pca', genes=[], seed=0):
        """
    ...
    def diff_exp_tradeseq(self, cutoff=1e-5, genes=[], gene_names=[], k=range(3,20),
                                        save_as=None, load_from=None, verbose=True):
        """
        association test for differential expression through pseudotime
        ...
        if load_from is not None:
            self.deg_tradeseq = pd.read_csv(load_from, index_col=0)
            ...
            X = self.raw_counts.loc[:, self.normalized.columns].T
            pt = np.expand_dims(self.pseudotimes.values, axis=1)

            if len(genes)>0:
                X = X.loc[np.intersect1d(X.index, genes),:]

            if len(k)>1:
                # determine the optimal number of knots for fitting each gene's model
                icMat = tradeseq.evaluateK(counts=X.values, pseudotime=pt,
                                           cellWeights=np.ones(pt.shape),
                                           k=np.array(k, dtype=int), verbose=verbose)

                k = list(int(input('Choose k: ')))

            # fit a general additive model for each gene's trajectory
            sce = tradeseq.fitGAM(counts=X.values, pseudotime=pt,
                                  cellWeights=np.ones(pt.shape),
                                  nknots=int(k[0]), verbose=False)

            # association test (DE through pseudotime)
            assoRes = tradeseq.associationTest(sce)
            assoRes.set_index(X.index, inplace=True)

            self.deg_tradeseq = assoRes.loc[assoRes['pvalue']<cutoff,:].copy()

            if len(gene_names)>0:
                indices = [genes.index(i) for i in self.deg_tradeseq.index]
                self.deg_tradeseq['gene_name'] = np.array(gene_names)[indices]

            if save_as is not None:
                self.deg_tradeseq.to_csv(save_as)
